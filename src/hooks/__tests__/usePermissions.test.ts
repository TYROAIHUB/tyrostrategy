...
describe("usePermissions", () => {
  ...
  describe("canCreateHedef", () => {
    it("Admin can create hedef", () => {
      setCurrentUser("Admin User", "Admin");
      const { result } = renderHook(() => usePermissions());
      expect(result.current.canCreateHedef).toBe(true);
    });

    it("Proje Lideri cannot create hedef", () => {
      setCurrentUser("Leader User", "Proje Lideri");
      const { result } = renderHook(() => usePermissions());
      expect(result.current.canCreateHedef).toBe(false);
    });

    it("Kullanıcı cannot create hedef", () => {
      setCurrentUser("Normal User", "Kullanıcı");
      const { result } = renderHook(() => usePermissions());
      expect(result.current.canCreateHedef).toBe(false);
    });
  ...
  describe("canDeleteHedef with cascade", () => {
    it("returns false when hedef has child projeler", () => {
      setCurrentUser("Admin User", "Admin");
      const { result } = renderHook(() => usePermissions());
    ...
    it("returns true when hedef has no child projeler", () => {
      setCurrentUser("Admin User", "Admin");
      // Remove all projeler
    ...
    it("Proje Lideri cannot delete hedef regardless of cascade", () => {
      setCurrentUser("Leader User", "Proje Lideri");
      useDataStore.setState({ projeler: [] });
      const { result } = renderHook(() => usePermissions());
  ...
  describe("filterHedefler", () => {
    it("Admin gets all hedefler", () => {
      setCurrentUser("Admin User", "Admin");
      const { result } = renderHook(() => usePermissions());
    ...
    it("Proje Lideri gets only hedefler they own/lead or linked via projeler", () => {
      setCurrentUser("Leader User", "Proje Lideri");
      const { result } = renderHook(() => usePermissions());
    ...
    it("Kullanıcı gets hedefler linked through assigned gorevler", () => {
      setCurrentUser("Normal User", "Kullanıcı");
      const { result } = renderHook(() => usePermissions());
